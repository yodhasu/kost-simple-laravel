...
const errorMessage = computed(() => {
    if (!hasRegionAccess.value) {
        return 'Anda tidak memiliki akses region untuk menambah pemasukan.';
    }
    if (!form.tingkat) {
        return 'Pilih tingkat pemasukan terlebih dahulu.';
    }
    if (!form.region_id) {
    ...
    if (!form.category) {
        return 'Tipe pemasukan wajib dipilih.';
    }
    if (form.amount <= 0) {
        return 'Jumlah pemasukan harus lebih besar dari 0.';
    }
...
<template>
    <BaseModal
        ...
        description="Isi pemasukan non-sewa seperti reimbursement, service, atau pemasukan operasional lain pada level region atau kost."
        max-width-class="sm:max-w-2xl"
        @update:open="emit('update:open', $event)"
    >
        <form id="income-form-modal" class="space-y-5" @submit.prevent="requestSave">
            <div class="grid gap-2">
                ...
                <p v-if="!hasRegionAccess" class="text-sm text-rose-300">
                    Anda tidak memiliki akses region untuk menambah pemasukan.
                </p>
            </div>
            ...
            <div class="grid gap-2">
                <Label class="text-slate-900">Tipe Pemasukan <span class="text-rose-500">*</span></Label>
                <select
                    v-model="form.category"
                    class="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900"
                >
                    <option value="" disabled class="text-slate-900">Pilih tipe pemasukan</option>
                    <option value="service" class="text-slate-900">Biaya Layanan</option>
                    <option value="utility_reimbursement" class="text-slate-900">Penggantian Utilitas</option>
                    <option value="late_fee" class="text-slate-900">Denda Keterlambatan</option>
                    <option value="parking" class="text-slate-900">Parkir</option>
                    <option value="laundry" class="text-slate-900">Laundry</option>
                    <option value="other_income" class="text-slate-900">Lainnya</option>
                </select>
            </div>
            ...
            <div class="grid gap-2">
                ...
                <textarea
                    ...
                    placeholder="Contoh: reimbursement listrik tenant kamar 07..."
                />
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div class="grid gap-2">
                    <Label class="text-slate-900">Jumlah Pemasukan <span class="text-rose-500">*</span></Label>
        ...
        <template #footer>
            ...
            <Button type="submit" form="income-form-modal" :disabled="Boolean(errorMessage)">
                Simpan Data
            </Button>
        </template>
    </BaseModal>
    ...
    <ConfirmModal
        ...
        :description="`Simpan pemasukan ${formatCurrency(form.amount)} untuk kategori ${form.category || '-'}?`"
