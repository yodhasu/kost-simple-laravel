...
class DataPurgeService
{
     ...
     */
    public function purgeRegion(Region $region): array
    {
     ...
     * @return array{tenantsPurged:int,transactionsPurged:int}
     */
    public function purgeKost(Kost $kost): array
    {
        return DB::transaction(function () use ($kost): array {
            $tenantIds = Tenant::query()->where('kost_id', $kost->id)->pluck('id');
            $tenantsPurged = $tenantIds->count();
